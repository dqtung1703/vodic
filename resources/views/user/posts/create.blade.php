...
<div class="container" style="padding: 3rem 1.5rem; max-width: 900px;">
    ...
    <form action="{{ route('user.posts.store') }}" method="POST" enctype="multipart/form-data" style="background: white; padding: 2rem; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
        @csrf
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            <textarea name="content" id="content" rows="15" required
                      style="width: 100%; padding: 0.75rem 1rem; border: 2px solid #e5e7eb; border-radius: 8px; font-size: 1rem; font-family: inherit;"
                      placeholder="Viết nội dung bài viết của bạn...">{{ old('content') }}</textarea>
            @error('content')
                <span style="color: #ef4444; font-size: 0.875rem; margin-top: 0.25rem; display: block;">{{ $message }}</span>
            @enderror
        </div>
